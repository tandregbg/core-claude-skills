...
**Capture the gap:** log an `edge_case` when a near-miss is flagged, a `correction` when the user resolves one. **Closing the loop is `/insights compile`'s job, not this skill's (CR-095).** Log the `edge_case` and move on -- compile's name-candidate roll-up is what surfaces a name flagged more than once, across folders, to someone who can decide whether it belongs in `people[]`. Do not write a roster entry from inside a transcript run: at that point only one occurrence is visible, and one occurrence is not evidence of a recurring person. Once a name is in `people[]`, this section goes quiet for it.
