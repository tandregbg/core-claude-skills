...
| CR-075 | `teams-chat-cli`, `_inbox/.audio/` | Medium | **Proposed — scope question sharpened 2026-09-24.** Fetch a meeting RECORDING into `_inbox/.audio/` with a paired stub (CR-012), so the existing chain transcribes it; Teams' own transcript is the lower-quality artefact and stays a backup lane. **Verified since filing:** recordings of meetings you ORGANISED are already reachable under `Files.Read` (32 of them, 3.9 GB, in your own OneDrive) — so the requested scopes buy meetings you only ATTENDED, which live in the organiser's drive and are not reachable at all. That is a narrower and more honest case than the CR first made. Needs `OnlineMeetings.Read` + `OnlineMeetingRecording.Read.All`, both delegated and user-scoped |
