...
| CR-084 | `ops` (`build_agenda.py`, `lint`), `ops-config` | **High** | **Proposed.** The agenda states its sources and their freshness first (note, chat, repo, tickets — `STALE`/`NOT DECLARED` printed, never omitted), ends with repo and ticket status per track (reads `.jirameta/`, which nothing read), checks carried items against the archives and lists likely-closed ones for confirmation, puts only items older than one session above the round, hands each person's track on from the previous note, and lints the roster against attendance and carried items that only a vault-local id names. Plus a stated skill-vs-project split |
