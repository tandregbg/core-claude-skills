...
|----|------|----------|---------|
| CR-069 | `ops-base` | Medium | Add a generic **Status/Results Report** standard beside TWO-TIER — measurement-driven (data source + period) vs the summary format's event-driven (transcript). Scorecard-first trend, partial-period guard, reconciliation invariants, honest provenance + blind spots, a warn-not-fail freshness gate, owner-ranked actions. Business-agnostic; producing skills reference it. Additive (21→22). |
